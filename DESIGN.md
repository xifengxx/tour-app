...
把每条导览做成一册正在展开的游记手卷——**古籍装帧的仪式感 × 现代编辑设计的克制**。用户打开的不是"一个旅游 App"，而是"一本带你去实地对读的书"。

**让人记住的那一件事：朱砂印章。** logo 是一方「巡」字朱印；收藏动作是"盖印"；每个导览卡角上压一枚小印。这是全站的签名动作。

这条理念落到所有设计决策上：

| 理念 | 落地 |
|------|------|
| 游记手卷 | 地图页底部卡片 = 手卷（卷轴杆拉手）；路线 narrative = 卷首语（楷体）；海报 = 一轴完整的卷（双线框 + 竖排侧签） |
| 古籍装帧 | 双线框、壹贰叁编号、篆刻步骤指示（壹贰叁）、竖排文字、纸纹 |
| 实地对读 | 小说场景 = 页边批注（朱色章节小标 + 藤黄「」楷体引文）；反思 =「驻足一想」朱砂批注 |
| 编辑的克制 | 亮色单色系（朱砂为主、黛青为辅）、地图标记只两态、圆角收敛、无多重阴影、留白充足 |
