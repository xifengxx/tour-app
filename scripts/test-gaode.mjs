...
function isScenicAnchor(loc, destName) {
  ...
  if (destName) { // v70：子串匹配须带设计词后缀，防"中国嵩山卢崖瀑布/青城山索道"成锚点
    ...
    if (n.startsWith(destName.slice(0, 2)) && /(后山|前山|西线|东线|南线|北线|北坡|南坡|西坡|东坡)(景区|风景区)?$/.test(n)) return true; // v70.2 卫星景区
  }
...
okLog(isScenicAnchor({ name: "青城后山" }, "青城山"), "v70.2 青城后山 → 锚点(卫星景区，后山子点可确定性扫描)");
okLog(!isScenicAnchor({ name: "青城山山门" }, "青城山"), "青城山山门 → 非锚点(非方位后缀)");
