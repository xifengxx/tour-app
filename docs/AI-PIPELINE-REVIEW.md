...
## 九、v70.2 ~ v70.4：残留问题修复 + 小说源场景验证（2026-08-06，已上线至 version 76）

承接 §八的三条线上残留，本轮全部修复并用两个 draft 导览（泰山封禅、课本里的黄山）完成单景区与小说源双场景验证。

### v70.2：卫星景区锚点 + stops 保序去重（version 73）

- **青城山 2 日 day-2 偏都江堰簇**（§八残留①）：根因是"青城后山"未被识别为锚点，进不了 1 日/2 日核心池。`isScenicAnchor` 新增**目的地前缀+方位后缀**卫星景区规则（"青城后山/黄山北坡"这类 `目的地+方位词` 命名直接成锚点）。修复后 day-2 = 泰安古镇 → 五龙沟（真后山）✓
- **天门山 1 日首末站重复**（§八残留②）：`stops` 数组保序去重（同名站点只保留首次出现），1 日 8 站无重复 ✓

### v70.3：文学巡礼线空回退 + 设施过滤补强（version 74）

- 文学巡礼线 resolve 结果为空时回退核心池 top4，避免小说源导览静默丢线
- `FACILITY_RE` 增补 `招商中心|营销中心|售楼处`；`cleanName` 剥除"（暂停开放）"类状态后缀
- 泰山重跑验证：混入的"招商中心"消失 ✓

### v70.4：文学巡礼线 dedup 豁免（version 76）—— 黄山 routes=3 根因

- **现象**：黄山（小说源《语文课本中的黄山》）应 4 条路线实际只有 3 条，文学巡礼线消失
- **诊断**：process_report 埋点（`plans`/`corePool`/`📖 resolve N 站` warning）两轮实锤——文学巡礼线 resolve 8 站成功，但 AI 自由选的 8 站与 1 日线 corePool top8 **集合完全相同**，被 stops-set 去重误判为重复路线删除
- **修复**：route 对象标记 `_free: !plan.allow`（文学巡礼线等自由选点路线），集合去重跳过 `_free` 路线——即使站点集合相同，只要叙事顺序/主题不同就保留

### 验证汇总（version 76 线上实测）

| 导览 | 场景 | 结果 |
|------|------|------|
| **青城山** | 多景区+卫星景区 | ✅ 2 日 day-2 = 泰安古镇→五龙沟（真后山），v70.2 修复生效 |
| **张家界天门山** | 双景区 32km | ✅ 1 日 8 站无重复；2 日武陵源全线（v70.1 阈值） |
| **泰山封禅** | 单景区+小说源 | ✅ 22 地点 / **4 路线**（含文学巡礼线）；招商中心被过滤 |
| **黄山（课本）** | 单景区+小说源 | ✅ 28 地点 / **4 路线** / 四层 28/28；文学巡礼线按文学叙事顺序排列（黄山→迎客松→光明顶→莲花峰→天都峰赋→西海大峡谷→始信峰→猴子观海），与 1 日线同集合不同序，dedup 豁免正确保留 |

### 仍存在的非确定性残留（不阻断）

- **地区合并是否触发有 AI 随机性**：黄山两次运行 26→17→22→28 地点漂移，取决于 DeepSeek 当次提议构成；质量均合格但不可复现
- **金鞭溪/水绕四门 around 扫描**：高德 around 接口仍扫不到，靠 AI 提议 + regionScenics 兜底（目前各轮均兜住）
- 小说源提取可能产出"天都峰赋"这类文学作品名作站点名——系小说源特性，非 bug
