  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-12">
        <div className="text-foreground font-serif font-bold text-lg mb-1">
          {'⭐'.repeat(loc.importance || 1)} {loc.name}
        </div>
        {loc.elevation && <div className="text-muted-foreground text-xs mb-3">🏔 {loc.elevation}</div>}

        {loc.tags && (
          <div className="flex gap-1 flex-wrap mb-3">
            {loc.tags.map((t, i) => (
              <span key={i} className="px-2 py-0.5 rounded-full bg-black/5 text-muted-foreground text-xs">{t}</span>
            ))}
          </div>
        )}
        ...
        {(() => {
          ...
          return <p className="text-muted-foreground text-sm leading-relaxed">{ld.text}</p>;
        })()}
        ...
        {loc.reflection && (
          <div className="bg-yellow-600/5 border border-yellow-600/15 rounded-xl p-3 my-3">
            ...
            <div className="text-[#7a6a4f] text-xs leading-relaxed">{loc.reflection}</div>
          </div>
        )}
        ...
        {loc.practical && (loc.practical.access || loc.practical.difficulty || loc.practical.tip) && (
          <div className="flex gap-1 flex-wrap my-2">
            {loc.practical.access && <span className="px-2 py-1 rounded-full bg-black/5 text-muted-foreground text-xs">🚶 {loc.practical.access}</span>}
            {loc.practical.difficulty && <span className="px-2 py-1 rounded-full bg-black/5 text-muted-foreground text-xs">⚡ {loc.practical.difficulty}</span>}
            {loc.practical.tip && <span className="px-2 py-1 rounded-full bg-black/5 text-muted-foreground text-xs">💡 {loc.practical.tip}</span>}
          </div>
        )}
        ...
        <div className="flex gap-2 mt-3">
          ...
          <button
            ...
            className="flex-1 py-3 bg-black/5 text-muted-foreground rounded-xl text-sm font-semibold"
          >
            📖 查看全文
          </button>
        </div>
